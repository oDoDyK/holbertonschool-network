...
Google edge firewalls Only allowed traffic to HTTPS services continues.
...
Client and server negotiate encryption keys. After this, HTTP traffic is encrypted.
5) Load balancer
...
Auth/session checks
Request processing
Building response payload (HTML/JSON) It may call internal services and databases.
8) Database
...
Server returns HTTP response over encrypted TLS connection. Browser then:
